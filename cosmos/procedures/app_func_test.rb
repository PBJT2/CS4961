...
module AppFuncTest
 ...
 #{Cosmos::Test.current_test_suite}:#{Cosmos::Test.current_test}:#{Cosmos::Test.current_test_case}"
      puts app.fsw_name
...
#################################
   ...
   def self.send_cmd(app, cmd_str, expect_event: true, error_case: false, hk_timeout: 8)
      ...
      cmd_seq_cnt   = tlm("#{target_hk_str} #{Ccsds::PRI_HDR_SEQUENCE}") #current seq count

      event_seq_cnt = tlm("CFE_EVS EVENT_MSG_PKT CCSDS_SEQUENCE") #initial seq count
      app.send_cmd(cmd_str) #send_cmd in flight.rb
	...
	else if (app.fsw_name.to_s.eql? "DOSD" or app.fsw_name.to_s.eql? "DOSI") #DOSD,DOSI doesn't need to send telemetry atm, just check event msg
	    expected_result = 
		   (cmd_error_cnt == tlm("#{target_hk_str} #{Osk::TLM_STR_CMD_ERR}")) and
		   (cmd_valid_cnt == tlm("#{target_hk_str} #{Osk::TLM_STR_CMD_VLD}") + 1)
