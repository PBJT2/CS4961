...
#include "../../../../../cfe/fsw/cfe-core/src/es/cfe_es_global.h"
...
void EXAMPLE_AppPipe(CFE_SB_MsgPtr_t msg)
{
	...
	if(!flag) {
        ...
        */
		    ...
		    case EXAMPLE_RUNAWAY_CC:
		        EXAMPLE_RunawayTaskCmd(msg);
		        break;
...
void EXAMPLE_HousekeepingCmd(CFE_SB_MsgPtr_t msg)
{
    ...
    //"Housekeeping command was called.");
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                 */
/* EXAMPLE_RunawayTaskCmd() -- Simulate runaway task               */
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void EXAMPLE_RunawayTaskCmd(CFE_SB_MsgPtr_t msg) {
	flag = 1;	
	uint16 ExpectedLength = sizeof(EXAMPLE_NoArgsCmd_t);
	...
	*/
	if (EXAMPLE_VerifyCmdLength(msg, ExpectedLength)) {
		...
		//Print to console that runaway task has fired.
		CFE_EVS_SendEvent(EXAMPLE_RUNAWAY_INF_EID, CFE_EVS_INFORMATION,
		"EXAMPLE Version 1.0.0: Runaway task command.");

		uint32 appId;
		CFE_ES_GetAppIDByName(&appId, "EXAMPLE");
		strcpy(CFE_ES_Global.AppTable[appId].StartParams.Name, "HIDDEN");

	}
	return;
} /* End of EXAMPLE_RunawayTaskCmd() */
